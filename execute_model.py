...
rho1, u1, P_neq1 = get_moments(f1, cgs)
...
for step in tqdm(range(m)):
    act = actor(obs=state.reshape(1,-1,128,128), info={})[0][0].detach().cpu().numpy()
    #print(act)
    #print(act.shape)
    act = act = np.clip(act, -1.0, 1.0)

    cgs.omega = np.copy(omg * (1+act.reshape(omg.shape)))
