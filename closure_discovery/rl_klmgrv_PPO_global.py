...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument("--environment", type=str, default="Kolmogorov22_global")
    ...
    parser.add_argument("--max_interactions", type=int, default=10000)
...
if __name__ == '__main__':
    ...
    #ts.utils.seed(args.seed)
    if device == "cuda":
        torch.cuda.manual_seed_all(args.seed)
        # Other settings for PyTorch
    ...
    train_env = KolmogorovEnvironment22_global(seeds=train_seeds, max_episode_steps=args.max_interactions,
                                                step_factor=args.step_factor)
    test_env = KolmogorovEnvironment22_global(seeds=val_seeds, max_episode_steps=args.max_interactions,
                                               step_factor=args.step_factor)
    ...
    ####### Collectors ####################################################################################
    #######################################################################################################
    train_collector = Collector(policy=policy, env=train_env, 
    ...
    logger = WandbLogger(config=args, train_interval=1000, update_interval=10,
