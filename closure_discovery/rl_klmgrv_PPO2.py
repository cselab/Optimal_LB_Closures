...
if __name__ == '__main__':
    ...
    #initialize networks
    if args.setup == "loc":
        actor = local_actor_net(in_channels=5, device=device).to(device)
        critic = central_critic_net2(in_channels=5, device=device).to(device)
    elif args.setup == "glob":
        actor = central_actor_net2(in_channels=5, device=device).to(device)
        critic = central_critic_net2(in_channels=5, device=device).to(device)
    elif args.setup == "interp":
        #actor = FullyConvNet_interpolating_agents(in_channels=6, N=args.num_agents, device=device).to(device)
        actor = FullyConvNet_interpolating_agents3(in_channels=5, N=args.num_agents, device=device).to(device)
        critic = central_critic_net2(in_channels=5, device=device).to(device)
