...
from tianshou.data import Batch, Collector, VectorReplayBuffer, ReplayBuffer
...
if __name__ == '__main__':
    ...
    train_env = KolmogorovEnvironment3(step_factor=args.step_factor,
                                      max_episode_steps=args.max_interactions,
                                      seeds=train_seeds,
                                      N_agents=args.num_agents,
                                      flow=args.environment)

    test_env = KolmogorovEnvironment3(seeds=val_seeds,
                                     max_episode_steps=args.max_interactions,
    ...
    #initialize networks
    if args.setup == "loc":
        actor = local_actor_net(in_channels=2, device=device).to(device)
        ...
        actor = FullyConvNet_interpolating_agents3(in_channels=2, N=args.num_agents, device=device).to(device)
        critic = central_critic_net2(in_channels=2, device=device).to(device)
