...
class FullyConvNet_interpolating_agents256(nn.Module):
    def __init__(self, N, in_channels=1, device="cpu", padding_mode="circular"):
        super().__init__()
        self.N = N
        self.device = device
        receptive_field = 256 // N  # Receptive field calculation based on N
    ...
    def forward(self, obs, state=None, info={}):
        ...
        # Pass through the model
        logits = self.model(obs.reshape(batch, -1, 256, 256))  # Reshape to correct input shape
        mu = self.mu(logits)
        sigma = self.sigma(logits)
...
#central critic for PPO
class central_critic_net256(nn.Module):
    ...
    def forward(self, obs, state=None, info={}):
        ...
        batch = obs.shape[0]
        obs = obs.reshape(batch,-1,256,256)
